...
export class GoogleKgSearchAdapter implements KnowledgeGraphAdapter<EntityType[]> {
	public async query(
	...
	private resolveUrl(term: string, types: EntityType[], limit: number): URL {
		const url = new URL(env.PUBLIC_GOOGLE_KNOWLEDGE_GRAPH_API_URL);
		url.searchParams.set('key', env.PUBLIC_GOOGLE_KNOWLEDGE_GRAPH_API_KEY);
	...
	private async request<T>(request: Request): Promise<T | undefined> {
		...
		try {
			...
			console.error(`Could not fetch data from Google Knowledge Graph API: ${message}`, {
				error: error.cause
			});
