...
import { EndOfStreamError, fromBuffer } from 'strtok3';
import type { BufferTokenizer } from 'strtok3/lib/BufferTokenizer';
...
async function fileTypeFromTokenizer( tokenizer: BufferTokenizer ): Promise<FileType | undefined> {
	try {
...
export function stringToBytes( string: string ): number[] {
	// eslint-disable-line unicorn/prefer-code-point
	return [ ...string ].map( ( character ) => character.charCodeAt( 0 ) );
}

export function tarHeaderChecksumMatches( buffer: Buffer, offset: number = 0 ): boolean {
	// Read sum in header
...
function _check( buffer: Uint8Array, headers: any, options?: Record<string, any> ): boolean {
	options = {
		offset: 0,
		...( options || {} )
	};
...
class FileTypeParser
{
	...
	minimumBytes: number = 4_100;

	check( headers: Iterable<number>, options?: Record<string, any> ) {
		return _check( this.buffer, headers, options );
	}

	checkString( header: string, options?: Record<string, any> ): boolean {
		return this.check( stringToBytes( header ), options );
	}

	async parse( tokenizer: BufferTokenizer ): Promise<FileType | undefined> {
		this.buffer = Buffer.alloc( this.minimumBytes );
		...
		if ( this.check( [ 0x50, 0x4b, 0x3, 0x4 ] ) ) {
			...
			try {
				while ( tokenizer.position + 30 < tokenizer.fileInfo.size ) {
					...
					const zipHeader: Record<string, any> = {
		...
		if ( this.check( [ 0x1a, 0x45, 0xdf, 0xa3 ] ) ) {
			...
			async function readChildren( children: number ): Promise<string | undefined> {
				while ( children > 0 ) {
			...
			const { len } = await readElement();
			const docType = await readChildren( len );
	...
	async readTiffTag( bigEndian: boolean ): Promise<FileType | undefined> {
	...
	async readTiffIFD( bigEndian: boolean ): Promise<FileType | undefined> {
	...
	async readTiffHeader( bigEndian: boolean ): Promise<FileType | undefined> {
