...
import { createStreamFromArrayBuffer } from "@project-kiosk/utils";
...
export class CloudflareAdapter implements StorageAdapter {
  ...
  async writeFile(
    ...
    await this.resolveBucket(platform).put(
      path,
      content as CloudflareContent<typeof content>
    );
  }
  ...
  private resolveBucket(
    ...
    if (typeof platform === "undefined" || !("env" in platform)) {
      throw new Error(
        "Cloudflare R2 adapter can only be used in Cloudflare Workers"
      );
    }

    const env: CloudflarePlatformEnv<typeof this.bucketName> = (platform as any)
      .env;
